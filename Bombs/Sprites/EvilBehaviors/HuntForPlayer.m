...
-(GameDirection)bestDirectionFrom:(CGPoint)startPosition to:(CGPoint)goalPosition{
    ...
    return [self bestDirectionFrom:startPosition to:goalPosition traversed:traversedTiles numCheckedTiles:numCheckedTiles];
}
-(GameDirection)bestDirectionFrom:(CGPoint)startPosition to:(CGPoint)goalPosition traversed:(bool[kBoardWidthInTiles][kBoardHeightInTiles])originalTraversedTiles numCheckedTiles:(int)numCheckedTiles{
    numCheckedTiles++;
    bool traversedTiles[kBoardWidthInTiles][kBoardHeightInTiles];
    for(int i = 0; i < kBoardWidthInTiles; i++){
        for(int e = 0; e < kBoardHeightInTiles; e++){
    ...
    for(int i = 0; i < 4; i++){
        ...
        if(CGPointEqualToPoint(positionToCheck, goalPosition) || (!blockMap[(int)positionToCheck.x][(int)positionToCheck.y] && 
                                                                  !traversedTiles[(int)positionToCheck.x][(int)positionToCheck.y] && 
                                                                  (numCheckedTiles > 10 || [self bestDirectionFrom:positionToCheck to:goalPosition traversed:traversedTiles numCheckedTiles:numCheckedTiles] != -1))){
